<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0E4429,50:006D32,100:39D353&height=180&section=header&text=AKSHAY%20RUSHI&fontSize=48&fontColor=ffffff&animation=fadeIn&fontAlignY=42&desc=BACKEND%20%2F%20CLOUD%20%2F%20AI&descAlignY=68&descSize=16&descColor=39D353" width="100%"/>

<br>

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=600&size=16&duration=2800&pause=1000&color=39D353&center=true&vCenter=true&width=760&lines=%3E+building+systems+that+survive+the+happy+path;%3E+backend+%2B+cloud+%2B+AI;%3E+currently+shipping+Layr+%26+Behavioural+Companion;%3E+AWS+Certified+Cloud+Practitioner;%3E+probably+debugging+something+right+now" alt="Terminal status animation"/>

<br>

<table>
<tr>
<td align="center">

<a href="https://linkedin.com/in/akshayrushi">
<img src="https://img.shields.io/badge/LINKEDIN-161B22?style=for-the-badge&logo=linkedin&logoColor=39D353"/>
</a>

</td>

<td align="center">

<a href="mailto:rushiakshay@gmail.com">
<img src="https://img.shields.io/badge/EMAIL-161B22?style=for-the-badge&logo=gmail&logoColor=39D353"/>
</a>

</td>

<td align="center">

<a href="https://akshayportfolio-f16.pages.dev/">
<img src="https://img.shields.io/badge/PORTFOLIO-161B22?style=for-the-badge&logo=googlechrome&logoColor=39D353"/>
</a>

</td>

<td align="center">

<img src="https://komarev.com/ghpvc/?username=PhantomChillz&style=for-the-badge&color=238636&label=VISITORS"/>

</td>
</tr>
</table>

</div>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0E4429,50:26A641,100:39D353&height=3&section=header" width="90%"/>

</div>

<br>

## / WHO I AM

<table width="100%">
<tr>

<td width="34%" valign="top" bgcolor="#0D1117">

<pre>
╭────────────────────────╮
│   SYSTEM PROFILE       │
├────────────────────────┤
│ curious      : yes     │
│ builder      : yes     │
│ ownership    : high    │
│ shipping     : active  │
│ learning     : always  │
│ sleep        : optional│
╰────────────────────────╯
</pre>

<br>

<img src="https://img.shields.io/badge/STATUS-SHIPPING-39D353?style=for-the-badge&labelColor=0D1117"/>

</td>

<td width="66%" valign="top" bgcolor="#161B22">

I build systems that solve **real problems**.

Backend at the core. Cloud for scale. AI to unlock intelligence.

I care about **backend architecture, scalable systems, cloud infrastructure and reliable APIs** — the parts that make software actually work when the happy path disappears.

I'm equally into AI: not just training models, but turning them into **useful products people actually use**.

I move fast, take ownership, and learn by building.

And yes — I won a **national-level prompting competition before "prompt engineering" became a LinkedIn buzzword.**

<br>

`BUILD → BREAK → LEARN → BUILD BETTER`

</td>

</tr>
</table>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:39D353,50:26A641,100:0E4429&height=3&section=header" width="90%"/>

</div>

<br>

## / CURRENTLY BUILDING

<table width="100%">
<tr>

<td width="50%" valign="top" bgcolor="#0D1117">

### <font color="#39D353">◈ LAYR</font>

**Integrity Infrastructure**

A drop-in integrity layer that attaches to existing assessment and application platforms instead of forcing them through a migration.

Adding trust and verification without ripping out what already works.

<br>

<img src="https://img.shields.io/badge/IN_DEVELOPMENT-39D353?style=flat-square&labelColor=0D1117"/>

</td>

<td width="50%" valign="top" bgcolor="#161B22">

### <font color="#26A641">◉ BEHAVIOURAL COMPANION</font>

**AI + IoT Platform**

An AI-driven behavioural analysis platform combining real-time sensor data with intelligent pattern analysis.

<br>

`AI` `ML` `IoT`

<br>

<img src="https://img.shields.io/badge/IN_DEVELOPMENT-26A641?style=flat-square&labelColor=161B22"/>

</td>

</tr>
</table>

<br>

<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=500&size=13&pause=1800&color=39D353&center=true&vCenter=true&width=650&lines=%5BOK%5D+backend+online;%5BOK%5D+cloud+infrastructure+online;%5BOK%5D+AI+experiments+running;%5BWARN%5D+sleep+module+not+found" alt="System status"/>

</div>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0E4429,50:26A641,100:39D353&height=3&section=header" width="90%"/>

</div>

<br>

## / EXPERIENCE

<table width="100%">
<tr>

<td width="100%" valign="top" bgcolor="#161B22">

### <font color="#39D353">◈</font> Backend & Cloud Engineer — Team Lead

**B Xpress Digital** &nbsp;·&nbsp; `Present`

Leading a **4-person engineering team** rebuilding a real-time order management system, working across backend architecture, cloud infrastructure and production reliability.

<br>

**What I work on**

- Designing and evolving **backend architecture, APIs and database models** for a production order-management platform.
- Driving the rebuild of a **real-time workflow**, improving how orders move through the system and how operational data is handled.
- Designing and managing **cloud infrastructure and deployments** with a focus on reliability, scalability and performance.
- Working across application and infrastructure boundaries rather than treating backend engineering and cloud architecture as separate problems.
- Making technical decisions and coordinating implementation across a **4-person engineering team**.
- Contributed to a **15% increase in order volume**, translating engineering improvements into measurable business impact.

<br>

`BACKEND ARCHITECTURE`
`API DESIGN`
`DATABASES`
`AWS`
`DEPLOYMENT`
`SCALABILITY`
`TEAM LEADERSHIP`

</td>

</tr>
</table>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:39D353,50:26A641,100:0E4429&height=3&section=header" width="90%"/>

</div>

<br>

## / TECH STACK

<table width="100%">
<tr>

<td width="20%" valign="top" bgcolor="#0D1117">

### BACKEND

<img src="https://skillicons.dev/icons?i=java,spring,python,django,flask&theme=dark"/>

<br><br>

`REST APIs` `JWT`

### FRONTEND

<img src="https://skillicons.dev/icons?i=js,react,bootstrap,html,css,tailwind&theme=dark"/>

</td>

<td width="20%" valign="top" bgcolor="#161B22">

### DATABASES

<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis&theme=dark"/>

<br><br>

### DEVOPS

<img src="https://skillicons.dev/icons?i=docker,nginx,linux,git,github&theme=dark"/>

</td>

<td width="30%" valign="top" bgcolor="#0D1117">

### AWS / CLOUD

<img src="https://skillicons.dev/icons?i=aws&theme=dark"/>

<br>

`EC2` `Lambda` `ECS` `VPC` `ALB`

`CloudFront` `Route 53` `NAT Gateway`

`S3` `RDS` `DynamoDB`

`IAM` `Cognito` `API Gateway`

`CloudWatch` `CloudTrail`

</td>

<td width="30%" valign="top" bgcolor="#161B22">

### AI / ML

<img src="https://skillicons.dev/icons?i=pytorch&theme=dark"/>

<br>

`scikit-learn`
`Hugging Face`
`Ollama`
`LangChain`
`OpenAI / GPT`

<br>

### PROMPTING / LLM

`Prompt Engineering`

`Few-shot Prompting`

`Chain-of-Thought`

`RAG`

`Embeddings`

`Semantic Search`

</td>

</tr>
</table>

<br>

<div align="center">

<img src="https://img.shields.io/badge/AWS_CERTIFIED_CLOUD_PRACTITIONER-238636?style=for-the-badge&logo=amazonaws&logoColor=white"/>
&nbsp;
<img src="https://img.shields.io/badge/CLF--C02-0D1117?style=for-the-badge&logo=amazonaws&logoColor=39D353"/>

</div>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0E4429,50:26A641,100:39D353&height=3&section=header" width="90%"/>

</div>

<br>

## / FEATURED PROJECTS

<table width="100%">
<tr>

<td width="33%" valign="top" bgcolor="#0D1117">

### <font color="#39D353">△ DISASTER PREDICTION SYSTEM</font>

ML-powered early-warning system analyzing environmental, weather and historical data for events like cyclones and earthquakes.

`Python` `ML` `AWS`

<br>

<a href="https://github.com/phantomChillz/disaster-prediction-system">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

<td width="33%" valign="top" bgcolor="#161B22">

### <font color="#26A641">◇ VELOCITY</font>

No-code / low-code DAG-based trading orchestration engine with cycle detection, encrypted credentials, and live/dry-run execution.

`Java` `Spring Boot` `MongoDB`

<br>

<a href="https://github.com/phantomChillz/Velocity_Trading-Orchestration-Platform">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

<td width="34%" valign="top" bgcolor="#0D1117">

### <font color="#39D353">◎ ATHENA</font>

AI knowledge hub with semantic document search using embeddings, cosine similarity, and RBAC-secured JWT authentication.

`Spring Boot` `React` `Flask`

<br>

<a href="https://github.com/phantomChillz/Athena">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

</tr>

<tr>

<td width="33%" valign="top" bgcolor="#161B22">

### <font color="#26A641">□ NOVA</font>

A Notion-inspired productivity platform built around pages, blocks and workspace abstractions.

`Spring Boot` `JPA` `REST`

<br>

<a href="https://github.com/phantomChillz/nova">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

<td width="33%" valign="top" bgcolor="#0D1117">

### <font color="#39D353">◉ ECOTRACK</font>

Personal finance platform that turns raw spending data into actionable financial insight.

`Django` `Python`

<br>

<a href="https://github.com/phantomChillz/EcoTrack">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

<td width="34%" valign="top" bgcolor="#161B22">

### <font color="#26A641">◈ WILD RYDES</font>

Fully serverless ride-sharing application built using Lambda, API Gateway, Cognito, DynamoDB and S3.

`AWS` `Serverless` `Lambda`

<br>

<a href="https://github.com/phantomChillz/wildrydes-site">
<img src="https://img.shields.io/badge/VIEW_PROJECT-238636?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</td>

</tr>
</table>

<br>

<details>
<summary><b>⌁ EXPAND PROJECT ARCHIVE</b></summary>

<br>

| PROJECT | DESCRIPTION | STACK |
|---|---|---|
| **FertilizeWise** | ML-based fertilizer recommendation system | `Django` `Python` `Decision Tree` |
| **Real-Time Chat** | Real-time messaging platform | `Spring Boot` `WebSockets` |
| **NewNewz** | News aggregation platform | `Django` `Mediastack` |

</details>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:39D353,50:26A641,100:0E4429&height=3&section=header" width="90%"/>

</div>

<br>

## / RECOGNITION

<table width="100%">
<tr>

<td width="20%" align="center" valign="top" bgcolor="#0D1117">

&nbsp;

### 🏆

**WINNER**

### Prompt-It 2025

<sub>National Prompting Hackathon</sub>

&nbsp;

</td>

<td width="20%" align="center" valign="top" bgcolor="#161B22">

&nbsp;

### 🥈

**RUNNER-UP**

### HackSavy 2025

<sub>National Hackathon · 200+ teams</sub>

&nbsp;

</td>

<td width="20%" align="center" valign="top" bgcolor="#0D1117">

&nbsp;

### 🏆

**WINNER**

### Technovanza 2024

<sub>Technical Quiz</sub>

&nbsp;

</td>

<td width="20%" align="center" valign="top" bgcolor="#161B22">

&nbsp;

### 🥈

**RUNNER-UP**

### TechSparada 2024

<sub>Technical Presentation</sub>

&nbsp;

</td>

<td width="20%" align="center" valign="top" bgcolor="#0D1117">

&nbsp;

### ✦

**TOP 1%**

### NPTEL

<sub>Software Project Management · Worldwide</sub>

&nbsp;

</td>

</tr>
</table>

<br>

<div align="center">

### 9.13 CGPA &nbsp;·&nbsp; AWS CERTIFIED &nbsp;·&nbsp; 3× PODIUM FINISHER &nbsp;·&nbsp; TOP 1% NPTEL

</div>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0E4429,50:26A641,100:39D353&height=3&section=header" width="90%"/>

</div>

<br>

## / GITHUB

<table width="100%">
<tr>

<td width="33%" align="center" bgcolor="#0D1117">

<a href="https://github.com/PhantomChillz?tab=repositories">
<img src="https://img.shields.io/badge/REPOSITORIES-161B22?style=for-the-badge&logo=github&logoColor=39D353"/>
</a>

</td>

<td width="34%" align="center" bgcolor="#161B22">

<a href="https://github.com/PhantomChillz?tab=followers">
<img src="https://img.shields.io/github/followers/PhantomChillz?style=for-the-badge&color=161B22&labelColor=161B22&logo=github&logoColor=39D353&label=FOLLOWERS"/>
</a>

</td>

<td width="33%" align="center" bgcolor="#0D1117">

<a href="https://github.com/PhantomChillz?tab=stars">
<img src="https://img.shields.io/badge/STARS-161B22?style=for-the-badge&logo=github&logoColor=39D353"/>
</a>

</td>

</tr>
</table>

<br>

<div align="center">

<pre>
[ SYSTEM ONLINE ]
Consistency. Code. Commitment.
</pre>

</div>

<br>

<div align="center">

<picture>

<source
  media="(prefers-color-scheme: dark)"
  srcset="https://raw.githubusercontent.com/PhantomChillz/PhantomChillz/output/github-snake-dark.svg"
/>

<source
  media="(prefers-color-scheme: light)"
  srcset="https://raw.githubusercontent.com/PhantomChillz/PhantomChillz/output/github-snake.svg"
/>

<img
  src="https://raw.githubusercontent.com/PhantomChillz/PhantomChillz/output/github-snake.svg"
  alt="GitHub Contribution Snake"
  width="95%"
/>

</picture>

</div>

<br>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:39D353,50:006D32,100:0E4429&height=120&section=footer" width="100%"/>

<code>Built with passion. Optimized for impact.</code>

<br><br>

<a href="mailto:rushiakshay@gmail.com">rushiakshay@gmail.com</a>
&nbsp;·&nbsp;
<a href="https://akshayportfolio-f16.pages.dev/">Portfolio</a>
&nbsp;·&nbsp;
<a href="https://linkedin.com/in/akshayrushi">LinkedIn</a>

</div>
